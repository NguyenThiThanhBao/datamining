...
if __name__ == '__main__':
    ...
    parser.add_option("--train", dest="conll_train",
                      help="Path to annotated CONLL train file", metavar="FILE", default="N/A")
    parser.add_option("--dev", dest="conll_dev",
                      help="Path to annotated CONLL dev file", metavar="FILE", default="N/A")
    parser.add_option("--test", dest="conll_test",
                      help="Path to CONLL test file", metavar="FILE", default="N/A")
    parser.add_option("--output", dest="conll_test_output",
                      help="File name for predicted output", metavar="FILE", default="N/A")
    parser.add_option("--prevectors", dest="external_embedding",
                      help="Pre-trained vector embeddings", metavar="FILE")
    parser.add_option("--params", dest="params", help="Parameters file",
                      metavar="FILE", default="model.params")
    parser.add_option("--model", dest="model",
                      help="Load/Save model file", metavar="FILE", default="model")
    parser.add_option("--wembedding", type="int",
                      dest="wembedding_dims", default=100)
    parser.add_option("--cembedding", type="int",
                      dest="cembedding_dims", default=50)
    parser.add_option("--pembedding", type="int",
                      dest="pembedding_dims", default=100)
    ...
    parser.add_option("--bibi-lstm", action="store_false",
                      dest="bibiFlag", default=True)
    ...
    if options.predictFlag:
        ...
        if os.path.isfile(os.path.join(options.output, options.params)) and \
            ...
            parser = learner.jPosDepLearner(
                words, pos, rels, w2i, c2i, stored_opt)
            parser.Load(os.path.join(options.output,
            ...
            if options.conll_dev != "N/A":
                ...
                for idSent, devSent in enumerate(devPredSents):
                    conll_devSent = [entry for entry in devSent if isinstance(
        ...
        for epoch in xrange(options.epochs):
            ...
            if options.conll_dev == "N/A":
                ...
                print("Highest POS&LAS: %.2f at epoch %d" %
                      (highestScore, eId))
